...
const gridSize = 40;
const generateEmptyGrid = () => Array.from({ length: gridSize }, () => Array(gridSize).fill(""));
...
const WordsAdventure = () => {
  ...
  const placeWordsInGrid = () => {
    ...
    crosswordWords.forEach((wordObj) => {
  ...
  // Timer setup
  useEffect(() => {
    placeWordsInGrid(); // Place words when the component mounts
    ...
    <div className="crossword-container">
      ...
      {currentWord && (
        <div className="input-section">
          ...
          <div className="hint-info">Hints used: {hintsUsed}/{currentWord.word.length}</div>
        </div>
      )}
...
//         if (col + word.length <= gridSize) { // Ensure it fits in the row
//           for (let i = 0; i < word.length; i++) {
//             newGrid[row][col + i] = word[i];
//           }
//         }
//       } else if (direction === "vertical") {
//         if (row + word.length <= gridSize) { // Ensure it fits in the column
//           for (let i = 0; i < word.length; i++) {
//             newGrid[row + i][col] = word[i];
//           }
//         }
...
//   // Start the timer when the component mounts or when the game starts
//   useEffect(() => {
...
//       alert("Time's up! The game is over.");
//       setIsRunning(false);
//       // You could add logic to calculate scores here
//     }
//   }, [isRunning, timer]);
...
//     useEffect(() => {
//         placeWordsInGrid();
//         setIsRunning(true); // Start the timer when the grid is set
//       }, []);
